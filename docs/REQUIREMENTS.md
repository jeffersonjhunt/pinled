...
| HW-3 | Chain logic SHOULD be LVC family at 3.3 V, for edge rate and drive into the multi-drop `CLK` bus. `74LV165A` is an acceptable substitute; `74HC165` is acceptable at ≤ 2 MHz and is the practical choice for a through-hole bench build. Since rev D there is no per-hop gate delay in the clock path, so family choice is no longer a chain-length correctness issue. | MVP |
