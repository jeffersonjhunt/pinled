...
| FR-SCAN-7 | The clock burst SHALL be generated by SPI + DMA (`SCLK`=`CLK`, `MISO`=`DATA`, mode 1, receive-only), not by a CPU loop. A DMA transaction is gapless, which is required: a mid-frame stall longer than ~6 us lets the activity detectors discharge and silently resets the chain. | MVP |
