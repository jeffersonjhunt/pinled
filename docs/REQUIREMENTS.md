...
| FR-SCAN-4 | A scan frame SHALL pulse `/MR` low, then issue **17*N** clocks, sampling `DATA` on each rising edge. Every 17th sample is a module's arm clock and SHALL be discarded; the rest are line (i mod 17)-1 of module i/17. Any module count 1..8 SHALL be legal. | MVP |
...
| FR-SCAN-7 | The clock burst SHOULD be generated by SPI + DMA (`SCLK`=`CLK`, `MISO`=`DATA`, mode 0, receive-only). `/MR` SHOULD be driven from `CS` with positive polarity so the frame reset is hardware-timed. Since rev C the chain holds no analog state, so a mid-frame stall is harmless and this is a performance choice rather than a correctness requirement. | MVP |
...
| FR-SCAN-10 | Each module SHALL spend its first received clock arming (`ENP` = `STARTED`, sampled synchronously) so that line 0 gets a full bit period before it is sampled. The driver SHALL discard those arm samples. | MVP |
...
| FR-DIAG-2 | The firmware SHALL be able to advance the scan at a human-visible rate (`PINLED_SCAN_STEP_MS`), logging each count and the counter state to expect. Valid on production hardware since rev C — the chain holds no analog state, so it can be clocked arbitrarily slowly. | MVP |
| FR-DIAG-3 | The firmware SHALL be able to park the counter on a single channel indefinitely (`PINLED_SCAN_HOLD_CH`) so every node from the counter outputs to the MCU pin is a static level a meter can read. Valid on production hardware since rev C. | MVP |
...
| HW-4 | Modules SHALL chain on a **5-pin** JST-SH harness: `VCC` (5 V), `GND`, `DATA`, `CLK`, `/MR`, in ~100 mm hops, up to 8 modules / 800 mm. `/MR` is bussed to every module and is the frame reset. | MVP |
...
| HW-10 | Bus arbitration SHALL be a registered `STARTED` latch (second half of the '109, `J`=1, cleared by `/MR`), not an RC activity detector. No analog time constant may appear anywhere in the chain. | MVP |
