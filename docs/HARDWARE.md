...
contract. **Reference schematics** for the three boards — base (S3 + first 16
channels), driver expansion module, and per-socket lamp board — are in
[`schematics/`](schematics/) as SVG, generated by `tools/gen_schematics.py`
from this document's statements; regenerate after changing either. The board
partition and the plain-'165 daisy chain were confirmed by the owner
2026-08-22 (previously this file's chaining description was inference).
